...
def main():
    ...
    pokemon = requests.get(f"{POKEMONURL}")
    pokemon = pokemon.json()
    pokemon_names = []

    for name in pokemon.get("results"):
        pokemon_names.append(name.get("name"))

    finishedlist = pokemon_names.copy()
    pokemon_names = {}
    pokemon_names["names"] = finishedlist
    

    pokemondf = pandas.DataFrame(pokemon_names)
    pokemondf.to_excel("pokemonnames.xlsx", index=False)
